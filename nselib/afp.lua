...
FILE_BITMAP = {
  ...
  LongName = 0x40,
  ShortName = 0x80,
  NodeId = 0x100, -- FileID(wireshark) , FILPBIT_FNUM(netatalk)
  DataForkSize = 0x200,
...
-- Serialize path of a given type
-- NB: For now the actual UTF-8 encoding is ignored
-- local function encode_path_new (path)
--   if path.type == PATH_TYPE.ShortName or path.type == PATH_TYPE.LongName then
--     return string.pack("Bs1", path.type, path.name)
--   elseif path.type == PATH_TYPE.UTF8Name then
--     return string.pack(">BI4s2", path.type, 0x08000103, path.name)
...
Proto = {
  ...
  fp_get_server_info = function(self)
    ...
    offsets.machine_type, offsets.afp_version_count,
        offsets.uam_count, offsets.volume_icon_and_mask, pos = string.unpack(">I2I2I2I2", packet.data)
    ...
    offsets.server_signature, offsets.network_addresses_count,
        offsets.directory_names_count, offsets.utf8_server_name, pos = string.unpack(">I2I2I2I2", packet.data, pos)
  ...
  fp_open_vol = function(self, bitmap, volume_name)
    ...
    -- 排除 attention
    -- local nsedebug = require "nsedebug"
    -- print(nsedebug.tostr(response:getPacketHeader().command))

    if response:getPacketHeader().command == 8 then
      response = self:read_fp_packet()
      if response:getErrorCode() ~= ERROR.FPNoErr then
        return response
      end
  ...
  fp_open_fork = function(self, flag, volume_id, did, file_bitmap, access_mode, path)
    ...
    -- wireshark 有问题，识别 file_bitmap 变成了 4 字节，其实是 2 字节。
  ...
  -- @param count number containing the number of bytes to be written
  -- @return response object
  fp_list_extattr = function(self, vol_id, did, file_bitmap, maxreply, path)
    local packet, response
    local data_offset = 0
    --  print(path)
    -- 有 1 字节 pad
    local data = string.pack(">BxI2I4I2xxxxxxI4", COMMAND.FPListextattr, vol_id, did, file_bitmap, maxreply)
        .. encode_path(path)
    ...
    local attr_name
    -- nsedebug.print_hex(response.packet.data)
    attr_name = string.unpack(">s4", response.packet.data, 3)
    response:setResult(attr_name)
    return response
  end,
  ...
  -- @param count number containing the number of bytes to be written
  -- @return response object
  fp_get_extattr = function(self, vol_id, did, file_bitmap, maxreply, path, attr_name)
    local packet, response
    local data_offset = 0

    local data = string.pack(">BxI2I4I2xxxxxxxxxxxxxxxxI4", COMMAND.FPGetextattr, vol_id, did, file_bitmap, maxreply)
        .. encode_path(path)

    if (#data & 1) == 1 then
      data = data .. " " .. string.pack(">s2", attr_name) -- pad
    else
      data = data .. string.pack(">s2", attr_name)
    end
    ...
    local attr_value
    attr_value = string.unpack(">s4", response.packet.data, 3)
    response:setResult(attr_value)
    return response
  end,
  ...
  -- @param count number containing the number of bytes to be written
  -- @return response object
  fp_remove_extattr = function(self, vol_id, did, file_bitmap, path, attr_name)
    local packet, response
    local data_offset = 0


    local data = string.pack(">BxI2I4I2", COMMAND.FPRemoveextattr, vol_id, did, file_bitmap)
        .. encode_path(path)
  ...
  fp_create_file = function(self, flag, vol_id, did, path)
    ...
    -- 排除 attention ( bug ，create_file 有时会同时返回两个 dsi header ,
    -- 考虑到返回的 reponse 也没进行什么操作，所以不排除 attention 了）
    local response = self:read_fp_packet()
    -- if response:getPacketHeader().command == 8 then
    --   response = self:read_fp_packet()
    --   if response:getErrorCode() ~= ERROR.FPNoErr then
    --     return response
  ...
  -- @param path string containing the path, including the volume, path and file name
  -- @return response object
  fp_add_appl = function(self, vol_id, did, creator, appltag, path)
    local packet
    local data_offset = 0

    -- 有 1 字节 pad
    local data = string.pack(">BxI2I4", COMMAND.FPAddAppl, vol_id, did)
        .. creator -- 4 字节 char
        .. appltag -- 4 字节 char
        .. encode_path(path)
  ...
  -- @param path string containing the path, including the volume, path and file name
  -- @return response object
  fp_add_icon = function(self, vol_id, fcreator, ftype, itype, itag, bsize)
    local packet
    local data_offset = 0
    --  print(path)
    -- 有 1 字节 pad
    local data = string.pack(">BxI2", COMMAND.FPAddIcon, vol_id)
        .. fcreator -- 4 字节 char
        .. ftype -- 2 字节 char
        .. itype -- 1 字节 char
        .. " " -- 1 字节 pad
        .. itag -- 2 字节 char
        .. string.pack(">I2", bsize)
  ...
  -- @param path string containing the path, including the volume, path and file name
  -- @return response object
  fp_open_dt = function(self, vol_id, did)
    local packet
    local data_offset = 0
    --  print(path)
    --  有 1 字节 pad
    local data = string.pack(">BxI2I4", COMMAND.FPOpenDt, vol_id, did)
...
Helper = {
  ...
  OpenSession = function(self, host, port)
    ...
    self.socket:set_timeout(500000) -- 修改
  ...
  Login = function(self, version, username, password, options)
  ...
  -- @param password (optional) string containing the user password
  -- @param options table containing additional options <code>uam</code>
  Login32 = function(self, version, username, password, options)
    local uam = (options and options.UAM) and options.UAM or "DHCAST128"
    local response
    ...
    if (username and uam == "DHCAST128") then
      response = self.proto:fp_login("AFP3.2", "DHCAST128", username, password)
    elseif (username) then
      return false, ("Unsupported UAM: %s"):format(uam)
    else
      response = self.proto:fp_login("AFP3.2", "No User Authent")
    end
  ...
  -- @return status boolean true on success, false on failure
  -- @return content string containing the file contents
  ReadFile2 = function(self, str_path)
    local status, response, fork, content, vol_name
    local offset, count, did = 0, 1024, 2
    local status, path, vol_id
    local p = Util.SplitPath(str_path)
    ...
    response = self.proto:fp_open_fork(2, vol_id, did, 0, ACCESS_MODE.Read, path)
    if response:getErrorCode() ~= ERROR.FPNoErr then
      return false, response:getErrorMessage()
    end
  ...
  -- @return status boolean true on success, false on failure
  -- @return content string containing the file contents
  ReadFile_header = function(self, str_path)
    local status, response, fork, content, vol_name
    local offset, count, did = 0, 1024, 2
    local status, path, vol_id
    local p = Util.SplitPath(str_path)
    ...
    path = { type = PATH_TYPE.LongName, name = p.file }
    -- 修改为4 ，打开 header .有两个参数代码标识，一个是 flag ，一个是 access 。都是可控的。
    response = self.proto:fp_open_fork(4, vol_id, did, 0, ACCESS_MODE.Read | ACCESS_MODE.Write, path)
    if response:getErrorCode() ~= ERROR.FPNoErr then
      return false, response:getErrorMessage()
    end
  ...
  --- addAppl a file to the AFP server
  --
  -- @param str_path string containing the AFP sharepoint, path and filename eg. HR/Documents/File.doc
  ...
  AddAppl = function(self, creator, appltag, str_path)
    local status, response, fork, content
    local offset, count = 1, 1024
    local status, vol_id, did, path
    local p = Util.SplitPath(str_path)

    -- 既要获取 vol_id ，又要获取 dit 需要用 WalkDirTree 去打开。底层是 fp_get_file_dir_parms
    status, response = self:WalkDirTree(p.dir)
    vol_id = response.VolumeId
    did = response.DirectoryId
    ...
    -- 直接发一个包就完事的了。
    -- print(nsedebug.tostr(response))
    status, response = self.proto:fp_add_appl(vol_id, did, creator, appltag, path)
    if not status then
  ...
  --- AddIcon a file to the AFP server
  --
  -- @param str_path string containing the AFP sharepoint, path and filename eg. HR/Documents/File.doc
  -- @param fdata string containing the data to write to the file
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  AddIcon = function(self, volume_name, fcreator, ftype, itype, itag, bsize)
    local status, response, fork, content
    local offset, count = 1, 1024
    local status, vol_id, did, path
    -- local p = Util.SplitPath( str_path )
    ...
    response = self.proto:fp_open_vol(VOL_BITMAP.ID, volume_name)
    if response:getErrorCode() ~= ERROR.FPNoErr then
      return false, response:getErrorMessage()
    end
    -- 只要 vol_id 的话，就可以只用 fp_open_vol 。
    vol_id = response.result.volume_id


    -- 直接发一个包就完事的了。
    -- print(nsedebug.tostr(response))
    status, response = self.proto:fp_add_icon(vol_id, fcreator, ftype, itype, itag, bsize)
    if not status then
      if (response.header.error_code ~= ERROR.FPObjectExists) then
        return false, response.header.error_msg
      end
  ...
  --- openDt a file to the AFP server
  --
  -- @param str_path string containing the AFP sharepoint, path and filename eg. HR/Documents/File.doc
  -- @param fdata string containing the data to write to the file
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  OpenDt = function(self, volume_name)
    local status, response, fork, content
    local offset, count = 1, 1024
    local status, vol_id, did, path


    -- 通过 WalkDirTree 得到 volumeId 和 DirectoryId ，内部调用的是 fp_get_file_dir_parms 。
    status, response = self:WalkDirTree(volume_name)
    vol_id = response.VolumeId
    did = response.DirectoryId
    ...
    -- 直接发一个包就完事的了。
    -- print(nsedebug.tostr(response))
    status, response = self.proto:fp_open_dt(vol_id, did)
    if not status then
      if (response.header.error_code ~= ERROR.FPObjectExists) then
        return false, response.header.error_msg
      end
  ...
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  WriteFile_new = function(self, str_path, fdata)
    local status, response, fork, content
    local offset, count = 1, 1024
    local status, vol_id, did, path
    local p = Util.SplitPath(str_path)
  ...
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  Listextattr = function(self, str_path)
    local status, response, fork, content
    local status, vol_id, did, path
    local p = Util.SplitPath(str_path)
    ...
    -- 直接发一个包就完事的了。
    local file_bitmap, maxreply = FILE_BITMAP.ALL, 1
    response = self.proto:fp_list_extattr(vol_id, did, file_bitmap, maxreply, path)
    -- print(nsedebug.tostr(response))
  ...
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  Getextattr = function(self, str_path, attr_name)
    local status, response, fork, content
    local status, vol_id, did, path
    local p = Util.SplitPath(str_path)
    ...
    -- 直接发一个包就完事的了。
    -- print(nsedebug.tostr(response))
    local file_bitmap, maxreply = FILE_BITMAP.ALL, 65535
    response = self.proto:fp_get_extattr(vol_id, did, file_bitmap, maxreply, path, attr_name)
    -- print(nsedebug.tostr(response))
  ...
  -- @return status boolean true on success, false on failure
  -- @return error string containing error message if status is false
  Removeextattr = function(self, str_path, attr_name)
    local response
    local status, vol_id, did, path
    local p = Util.SplitPath(str_path)
    ...
    -- print(nsedebug.tostr(response))
    local file_bitmap, maxreply = FILE_BITMAP.ALL, 65535
    response = self.proto:fp_remove_extattr(vol_id, did, file_bitmap, path, attr_name)
  ...
  -- @return status boolean true on success, false on failure
  -- @return acls table containing the volume acls as returned by <code>acls_to_long_string</code>
  GetFinderInfo = function(self, str_path)
    local status, response, acls
    local p = Util.SplitPath(str_path)

    response = self.proto:fp_open_vol(VOL_BITMAP.ID, p.dir)
    ...
    if response:getErrorCode() == ERROR.FPNoErr then
      ...
      -- response = self.proto:fp_get_file_dir_parms(vol_id, 2, FILE_BITMAP.ALL, DIR_BITMAP.ALL, path)
      response = self.proto:fp_get_file_dir_parms(vol_id, 2, FILE_BITMAP.FinderInfo, DIR_BITMAP.ALL, path)
      if response:getErrorCode() == ERROR.FPNoErr then
        if (response.result.dir and response.result.dir.AccessRights) then
          acls = Util.acls_to_long_string(response.result.dir.AccessRights)
          acls.name = nil
