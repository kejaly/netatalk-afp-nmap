...
action = function(host, port)
  ...
  for username, password in pairs(users) do
    ...
    if ( username ~= 'nil' ) then
      status, response = afpHelper:Login32(username, password)
    else
      status, response = afpHelper:Login32()
    end
    ...
    if(not(rpath)) then
      return false,"need attr_name arg"
    end
    ...
    local msg
    status, msg = afpHelper:Removeextattr(rpath,attr_name)  -- for adouble file
    -- if flag=="1" then 
  ...
  return "agp-removeextattr empty"
end
